...
def test_refresh_token(client, login_details):
    ...
    response = client.post(path=endpoint, data={
                           'refresh': login_details[1]}, HTTP_AUTHORIZATION='Bearer ' + login_details[0])
    print(response.data)
...
def test_invalid_refresh_token(client, login_details):
    endpoint = reverse('token-refresh')
    response = client.post(path=endpoint, data={
                           'refresh': ''}, HTTP_AUTHORIZATION='Bearer '+login_details[0])

    print(response.data)
    assert response.data['refresh'][0].__str__(
